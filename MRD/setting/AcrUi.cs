...
public class AcrUi
{
    ...
    public void Draw日志(JobViewWindow jobViewWindow)
    {
        ...
        if (ImGui.Button("你想看看涩图么？")) 
        {
            ...
            Process.Start(new ProcessStartInfo("cmd", "/c start https://discord.com/channels/1191648233454313482/1196081001407979600"));
        }
        ImGui.Text("记得打开梯子哦");
    ...
    public void DrawGeneral(JobViewWindow jobViewWindow) //通用，会在WarriorRotationEntry中调用
    {
            ...
            if (ImGui.SliderInt("飞斧距离", ref MRD设置.Instance.飞斧距离, 5, 20))
            {
                MRD设置.Instance.Save();
            }
    ...
    public void Draw减伤(JobViewWindow jobViewWindow)
    {
        ...
        {
            if (ImGui.Checkbox("自动铁壁", ref MRD设置.Instance.自动铁壁)) MRD设置.Instance.Save();
            if (ImGui.Checkbox("自动复仇", ref MRD设置.Instance.自动复仇)) MRD设置.Instance.Save();
            if (ImGui.Checkbox("自动血仇", ref MRD设置.Instance.自动血仇)) MRD设置.Instance.Save();

            if (ImGui.Checkbox("自动战栗", ref MRD设置.Instance.自动战栗)) MRD设置.Instance.Save();
            if (ImGui.Checkbox("自动死斗", ref MRD设置.Instance.自动死斗)) MRD设置.Instance.Save();
        }
        ...
        if (ImGui.CollapsingHeader("自动减伤阈值"))
        {
            ...
            ImGui.Text("拖动滑块可自行调整");
            ImGui.Text("Ctrl+左键单击滑块可以直接输入数字");
            if (ImGui.SliderFloat("铁壁阈值", ref MRD设置.Instance.铁壁阈值, 0.0f, 0.99f))
            {
                MRD设置.Instance.Save();
            }

            if (ImGui.SliderFloat("复仇阈值", ref MRD设置.Instance.复仇阈值, 0.0f, 0.99f))
            {
                MRD设置.Instance.Save();
            }

            if (ImGui.SliderFloat("血仇阈值", ref MRD设置.Instance.血仇阈值, 0.0f, 0.99f))
            {
                MRD设置.Instance.Save();
            }
            ...
            if (ImGui.SliderFloat("战栗阈值", ref MRD设置.Instance.战栗阈值, 0.0f, 0.99f))
            {
                MRD设置.Instance.Save();
            }

            if (ImGui.SliderFloat("死斗阈值", ref MRD设置.Instance.死斗阈值, 0.0f, 0.99f))
            {
                MRD设置.Instance.Save();
            }

            if (ImGui.SliderFloat("血气阈值", ref MRD设置.Instance.铁壁阈值, 0.0f, 0.99f))
            {
                MRD设置.Instance.Save();
            }
        }
