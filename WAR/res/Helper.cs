...
public static class Helper
{
    ...
    */
    ...
    public static bool 是否AOE(CharacterAgent targer ,int timeleft)
    {
        //判断是否为AOE技能
        int[] numbers = Bossaoelist.list;
        int index = -1;
        if (targer.IsCasting)
        {
            index = Array.IndexOf(numbers, (int)Core.Me.GetCurrTarget().CastingSpellId);
            if (index != -1 && targer.TotalCastTime - targer.CurrentCastTime<timeleft)
            {
                return true;
            }
            else
    ...
    public static bool 是否死刑(CharacterAgent targer ,int timeleft)
    {
        //判断是否为死刑技能
        int[] numbers = DeathSentencelist.list;
        int index = -1;
        if (targer.IsCasting)
        {
            index = Array.IndexOf(numbers, (int)Core.Me.GetCurrTarget().CastingSpellId);
